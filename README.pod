...
There is no makefile or further installation instructions. You need to be familiar with Verilator,
as you need to add the jtag_dpi.cpp file to the generated C++ code. There are a few alternatives:

  1) Add the jtag_dpi.cpp to the Verilator command line.
  2) Include jtag_dpi.cpp from your main .cpp file (with #include).
  3) Edit the makefile you are using.
